/* ============================================================
    ...
    function pfp(seed, gender) { return `https://randomuser.me/api/portraits/${gender}/${seed}.jpg` }
    ...
    const jetImg = (i) => `https://images.unsplash.com/${['photo-1540962351504-03099e0a754b', 'photo-1474302770737-173ee21bab63', 'photo-1635672033263-a19f27eaefa8', 'photo-1619659085985-f51a00f0160a'][i % 4]}?w=600&h=400&fit=crop&auto=format`;
    const assetsDb = [
    ...
    function generateAdminDocs(listingId, isVerified) {
      const templates = [
    ...
    const ROLE_LABELS = { MASTER_ADMIN: 'Master Admin', GENERAL_ADMIN: 'General Admin', CUSTOMER_CARE_ADMIN: 'Customer Care Admin', BD_ADMIN: 'BD Admin', EXECUTIVE_ADMIN: 'Executive Admin', TECH_ADMIN: 'Tech Admin', FINANCE_ADMIN: 'Finance Admin', LISTING_ADMIN: 'Listing Admin', VERIFICATION_ADMIN: 'Verification Admin' };
    const ROLE_SCOPE = {
    ...
    const adminVerificationService = (() => {
      ...
      function _generateCode() { return String(Math.floor(100000 + Math.random() * 900000)); }
      ...
      function _getMasterAdmin() { return admins.find((a) => a.role === 'MASTER_ADMIN' && a.status === 'ACTIVE'); }
      ...
      return {
        ...
        async verifyMasterAdminCode(sessionToken, code) {
          _purgeExpired();
          const session = _sessions.get(sessionToken);
        ...
        async sendNewAdminCode(sessionToken, email) {
          _purgeExpired();
          const session = _sessions.get(sessionToken);
        ...
        async verifyNewAdminCode(sessionToken, code) {
          _purgeExpired();
          const session = _sessions.get(sessionToken);
        ...
        async resendMasterAdminCode(sessionToken) {
          _purgeExpired();
          const session = _sessions.get(sessionToken);
        ...
        async resendNewAdminCode(sessionToken) {
          _purgeExpired();
          const session = _sessions.get(sessionToken);
        ...
        async createAdmin(sessionToken, adminData) {
          _purgeExpired();
          const session = _sessions.get(sessionToken);
        ...
        destroySession(sessionToken) { if (sessionToken) _sessions.delete(sessionToken); }
      };
    })();

    const authService = {
      getQueryParam(name) { const params = new URLSearchParams(window.location.search); return params.get(name) },
      loadCurrentAdmin() {
        ...
        const permissions = admin.role === 'MASTER_ADMIN' ? ['*'] : Array.from(new Set([...scopePerms, ...(admin.permissions || []).filter((p) => scopePerms.includes(p) || p === '*')]));
        return Object.freeze({ ...admin, permissions, isMaster: admin.role === 'MASTER_ADMIN', homeRoute: ROLE_ROUTE_MAP[admin.role] || '/admin/dashboard', scope: ROLE_SCOPE[admin.role] || admin.role.toLowerCase() });
      },
      requireAuth() { return currentAdmin ? { ok: true, admin: currentAdmin } : { ok: false, reason: 'unauthenticated' }; },
      isMasterAdmin() { return currentAdmin?.isMaster === true; },
      ...
      getHomeRoute() { return currentAdmin?.homeRoute || '/admin/dashboard'; }
    };
    ...
    const auditLogService = (() => {
      ...
      return {
        log(action, target, result = 'Success') { logs.unshift({ timestamp: stamp(), admin: currentAdmin?.name || 'System', action, target, result }); },
    ...
    const authorizationService = {
      requireAuth() { return authService.requireAuth(); },
      requireRole(scope) {
    ...
    const adminRouter = {
      ...
      renderForbidden(homeRoute) {
        this.hideAllViews();
        byId('forbiddenView')?.classList.remove('hidden');
        closeModulePanel();
        byId('forbiddenReturnBtn').onclick = () => this.navigate(homeRoute || authService.getHomeRoute());
      },
      bind() {
        window.addEventListener('hashchange', () => this.handleRoute());
        window.addEventListener('popstate', () => this.handleRoute());
      },
      redirectToHome() { this.navigate(authService.getHomeRoute(), true); }
    };

    function setAdminPageTitle(guard) {
      if (!guard) return;
    ...
    function masterViewAdminDashboard(adminOrRole) {
      if (!authService.isMasterAdmin()) return;
      const role = typeof adminOrRole === 'string' ? adminOrRole : adminOrRole.role;
      const name = typeof adminOrRole === 'string' ? ROLE_LABELS[adminOrRole] : adminOrRole.name;
      closeModulePanel();
      ...
      if (!route) return;
      state.masterPreview = { role, name, id: adminOrRole.id || null };
      adminRouter.navigate(route);
    }

    function bindAdminManagementActions(wrap) {
      wrap.addEventListener('click', (e) => {
        const viewDash = e.target.closest('[data-view-dash]');
        const rm = e.target.closest('[data-remove-admin]');
        const toggle = e.target.closest('[data-toggle-admin]');
        const edit = e.target.closest('[data-edit-admin]');
        const perm = e.target.closest('[data-perm-admin]');
        ...
        if (!id) return;
        const target = admins.find((x) => x.id === id);
        if (!target) { showToast('Admin not found.'); return; }
        if (!authService.isMasterAdmin()) { adminRouter.renderForbidden(authService.getHomeRoute()); return; }
        if (viewDash) { masterViewAdminDashboard(target); return; }
        if (edit) { openAdminEditor(target); return; }
        if (perm) { openPermissionsEditor(target); return; }
        if (toggle) { toggleAdminStatus(target); return; }
        if (rm) { confirmAdminRemoval(target); return; }
      });
    }
    ...
    function openOverlay() { byId('overlay').classList.add('show') }
  ...
  `).join('');
      ...
      byId('statRow').querySelectorAll('[data-nav-listings]').forEach(el => {
    ...
    function renderDashboardApprovalsWidget() {
    ...
    <div class="dash-widget-list">
      ...
      widget.querySelectorAll('[data-quick-review]').forEach(b => {
      ...
      widget.querySelectorAll('[data-quick-verify]').forEach(b => {
        b.addEventListener('click', () => {
    ...
    function renderTicker() {
      ...
      byId('notifPrev').disabled = state.notifIndex === 0;
      byId('notifNext').disabled = state.notifIndex === notifications.length - 1;
    }
    ...
    function bindDashboard() {
      ...
      byId('graphToggle').addEventListener('click', (e) => {
        const btn = e.target.closest('button[data-period]');
        if (!btn) return;
        state.graphPeriod = btn.dataset.period;
    ...
    ============================================================ */
    function renderTable(columns, rows, onRowClick, emptyText) {
      ...
      const body = rows.map((row, idx) => `<tr data-row-idx="${idx}">${columns.map((c) => `<td class="${c.muted ? 'muted-cell' : ''}">${c.render ? c.render(row) : esc(row[c.key] ?? '')}</td>`).join('')}</tr>`).join('');
      ...
      if (onRowClick) {
        wrap.addEventListener('click', (e) => {
          ...
          if (!tr || e.target.closest('.row-edit-btn')) return;
    ...
    function bindSheet(containerEl, rows, onRowClick, onEditClick) {
      containerEl.addEventListener('click', (e) => {
        const editBtn = e.target.closest('.row-edit-btn');
        if (editBtn) { onEditClick && onEditClick(rows[Number(editBtn.dataset.editIdx)]); return }
        const tr = e.target.closest('tr[data-row-idx]');
    ...
    /* ============================================================
       MODULE: DATABASES
    ============================================================ */
    function renderDatabasesModule() {
      const body = byId('moduleBody');
      ...
      <button class="hub-block" data-db-sub="${h.key}">
        ...
        body.querySelectorAll('[data-db-sub]').forEach((b) => b.addEventListener('click', () => { state.tabs.databases = b.dataset.dbSub; renderModuleBody('databases') }));
        return;
      }
      ...
      <div class="sheet-wrap" id="usersTableWrap">${renderTable(
        ...
        byId('usersTableWrap').replaceWith((() => {
          const d = document.createElement('div'); d.id = 'usersTableWrap'; d.innerHTML = renderTable(
            [{ key: 'name', label: 'Username' }, { key: 'company', label: 'Company', muted: true }, { key: 'email', label: 'Email', muted: true }, { key: 'phone', label: 'Phone', muted: true }],
        ...
        byId('userSearch').addEventListener('input', (e) => { state.search.users = e.target.value; renderModuleBody('databases') });
      }
      if (sub === 'partners') {
        body.innerHTML = `${backBtn}
      ...
      <div class="sheet-wrap" id="partnersWrap"></div>`;
        byId('partnersWrap').innerHTML = renderTable(
      ...
      <div class="sheet-wrap" id="assetsWrap"></div>`;
        const cols = [{ key: 'manufacturer', label: 'Manufacturer' }, { key: 'model', label: 'Model' }, { key: 'type', label: 'Jet Type', muted: true }, { key: 'passengers', label: 'Passengers', muted: true },
        ...
        byId('assetsWrap').innerHTML = renderTable(cols, assetsDb, null);
      ...
      if (sub === 'inventory') {
        ...
        byId('invWrap').innerHTML = renderTable([{ key: 'owner', label: 'Owner' }, { key: 'asset', label: 'Asset' }, { key: 'since', label: 'Since', muted: true }, { key: 'status', label: 'Status', muted: true }], inventory, null);
        byId('dbBackToHub').addEventListener('click', () => { state.tabs.databases = 'hub'; renderModuleBody('databases') });
      }
      if (sub === 'offmarket') {
        body.innerHTML = `${backBtn}<div class="panel-head"><h3>Off-Market Database</h3></div><div class="sheet-wrap" id="omWrap"></div>`;
        byId('omWrap').innerHTML = renderTable([{ key: 'name', label: 'Asset' }, { key: 'owner', label: 'Owner' }, { key: 'ask', label: 'Ask', muted: true }, { key: 'status', label: 'Status', muted: true }], offMarket, null);
        byId('dbBackToHub').addEventListener('click', () => { state.tabs.databases = 'hub'; renderModuleBody('databases') });
      }
    }
    function openUserSummary(u) {
      openModal(`
    ...
    <div class="modal-grid">
      ...
      byId('detailModal').querySelectorAll('[data-deep]').forEach((btn) => {
      ...
      const revealBtn = byId('detailModal').querySelector('#revealCredsBtn');
      if (revealBtn) {
    ...
    ============================================================ */
    function renderEAcquisitionModule() {
      ...
      byId('eaTabs').addEventListener('click', (e) => { const b = e.target.closest('[data-tab]'); if (!b) return; state.tabs.eacquisition = b.dataset.tab; renderModuleBody('eacquisition') });
      ...
      if (tab === 'leads') {
        ...
        byId('leadsWrap').innerHTML = renderTable([{ key: 'name', label: 'Name' }, { key: 'model', label: 'Plane model' }, { key: 'pax', label: 'Pax', muted: true }, { key: 'phone', label: 'Phone', muted: true }], leads, null);
      ...
      if (tab === 'wall') {
        ...
        byId('wallWrap').innerHTML = renderTable([{ key: 'partner', label: 'Partner' }, { key: 'hours', label: 'Usage hours', muted: true }], m1wall, null);
      }
      if (tab === 'efficiency') {
        eaBody.innerHTML = `
      ...
      <div id="loadTestResult" style="margin-top:16px"></div>`;
        byId('runLoadTest').addEventListener('click', (e) => {
          e.target.disabled = true; e.target.textContent = 'Running…';
          showToast('Firing 10 concurrent engine requests…');
          setTimeout(() => {
            e.target.disabled = false; e.target.textContent = 'Run 10-request load test';
            ...
            byId('loadTestResult').innerHTML = `<div class="sheet-wrap">${renderTable([{ key: 'req', label: 'Request' }, { key: 'latency', label: 'Latency', muted: true }, { key: 'status', label: 'Status', render: (r) => `<span class="chip ${r.status === 'OK' ? 'ok' : 'warn'}">${r.status}</span>` }], rows, null)}</div>`;
    ...
    function renderDataFetchingModule() {
      ...
      byId('dfTabs').addEventListener('click', (e) => { const b = e.target.closest('[data-tab]'); if (!b) return; state.tabs.datafetching = b.dataset.tab; renderModuleBody('datafetching') });
      ...
      if (tab === 'news') {
        ...
        byId('newsWrap').innerHTML = renderTable([{ key: 'heading', label: 'Heading' }, { key: 'body', label: 'Body', muted: true },
        ...
        byId('newsWrap').addEventListener('click', (e) => {
        ...
        <button class="btn btn-primary" id="saveNewsBtn">Publish</button>`);
          byId('saveNewsBtn').addEventListener('click', () => {
            const heading = byId('newsHeading').value.trim();
            if (!heading) { showToast('Heading is required.'); return }
            newsItems.push({ id: 'n' + Date.now(), heading, body: byId('newsBody').value.trim(), image: byId('newsImage').value.trim(), link: byId('newsLink').value.trim() || '#' });
    ...
    ============================================================ */
    function renderThreeDotMenu(row) {
      return `
    ...
    function renderFlagDot(r) {
      const colorMap = { blue: '#70b5f9', red: '#ff5c5c', pink: '#e88fc4', yellow: '#f2c46d', green: '#3dd598' };
    ...
    function renderListingsHub() {
      const body = byId('moduleBody');
      if (!body) return;
      body.innerHTML = '';
    }

    function renderListingsModule() {
      try {
        const viewToggle = state.tabs.listingsToggle || 'verified';
        const body = byId('moduleBody');
        if (!body) return;
      ...
      <div id="listBody"></div>`;
        ...
        if (searchInput) {
          searchInput.addEventListener('input', (e) => {
            state.search.listings = e.target.value;
            renderModuleBody(state.currentModule || 'verifications');
          });
        }
        ...
        if (viewToggle === 'verified') {
          ...
          const cols = [
            ...
            { key: 'status', label: 'Status', render: (r) => `<span class="chip ok">✓ Verified</span>` },
        ...
        <div id="verifiedWrap">${renderTable(cols, rows, null, 'No verified listings match your search.')}</div>`;
          ...
          const cols = [
            ...
            { key: 'docs', label: 'Documents', render: (r) => `<span class="chip">📁 ${r.docs ? r.docs.length : 25} Docs</span>` },
            { key: 'status', label: 'Status', render: (r) => `<span class="chip warn">⏳ Pending Approval</span>` },
            { key: 'actions', label: 'Actions', render: (r) => renderThreeDotMenu(r) }
          ];

          lb.innerHTML = `<div class="panel-head"><h3>Approvals Queue</h3><span class="meta">${rows.length} pending verification</span></div>
        <div id="apprWrap">${renderTable(cols, rows, null, 'No pending approval requests.')}</div>`;
        ...
        <div id="incWrap">${renderTable(cols, rows, null, 'No incomplete draft listings.')}</div>`;
          byId('incWrap').querySelectorAll('[data-contact-incomplete]').forEach(b => {
            b.addEventListener('click', () => {
    ...
    function bindVerificationShiftArrows(viewToggle) {
      document.querySelectorAll('.verif-shift-arrow').forEach((el) => el.remove());
    ...
    function bindListingRowActions(wrapEl, rows) {
      if (!wrapEl) return;
      ...
      wrapEl.addEventListener('click', (e) => {
        const dotBtn = e.target.closest('[data-dot-toggle]');
        ...
        if (actionBtn) {
          ...
          const item = listings.find((l) => l.id === id) || approvals.find((a) => a.id === id);
          if (!item) return;
    ...
    async function executeDeclareVerified(item) {
    ...
    async function executeUnpublishListing(item) {
    ...
    function getListingGallery(item) {
      ...
      const imgs = (start) => [0, 1, 2, 3].map((i) => jetImg((i + start) % 4));
      return {
    ...
    function getListingAssetSpecs(item) {
      const catalog = {
    ...
    function getAirworthinessSpecs(item) {
    ...
    function initListingGallery(gallery) {
      const panel = byId('listingGalleryPanel');
      if (!panel) return;
      let mediaTab = 'exterior';
      let slideIdx = 0;
      ...
      <div class="listing-gallery-viewport">
        ...
        panel.querySelector('#listingGalleryTabs')?.addEventListener('click', (e) => {
        ...
        panel.querySelectorAll('[data-gallery-dot]').forEach((dot) => {
    ...
    function openAdminListingModal(item, initialTab = 'info') {
  ...
  `);
        ...
        <div class="listing-modal-split">
          <div class="listing-spec-panel">
            ...
            <div class="listing-overview-grid">
              ...
              <div class="item"><span>Verification Date</span><strong>${esc(item.verifiedDate || 'N/A')}</strong></div>
            </div>
      ...
      `).join('');
        ...
        <div class="listing-tab-full">
          ...
          box.onclick = (e) => {
            ...
            const verBtn = e.target.closest('[data-verify-doc]');
            if (viewBtn) {
      ...
      byId('adminModalTabs')?.addEventListener('click', (e) => {
    ...
    async function executeVerifyDoc(listingId, doc, callback) {
      doc.status = 'Verified';
    ...
    function openDocumentPreviewModal(doc, assetName, listingId) {
      openModal(`
  ...
  `);
    ...
    ============================================================ */
    function renderTabbedRequestsModule(moduleId, title, dataset, fields) {
      ...
      byId(`${moduleId}Tabs`).addEventListener('click', (e) => { const b = e.target.closest('[data-tab]'); if (!b) return; state.tabs[moduleId] = b.dataset.tab; renderModuleBody(moduleId) });
      ...
      byId(`${moduleId}Wrap`).innerHTML = renderTable(fields, rows, null, 'Nothing here yet.');
      bindSheet(byId(`${moduleId}Wrap`), rows, (r) => openModal(`
    <h3>${esc(r.name)}</h3><div class="sub">Owner: ${esc(r.owner)}</div>
    <div class="modal-card"><h4>Details</h4><div class="detail-list">${Object.entries(r).filter(([k]) => k !== 'id').map(([k, v]) => `<div class="item"><span>${esc(k)}</span><strong>${esc(v)}</strong></div>`).join('')}</div></div>`));
    }
    function renderVerificationsModule() {
      renderListingsModule();
    }
    function renderFeaturingModule() {
      renderTabbedRequestsModule('featuring', 'Featured Listings', featuring,
    ...
    function renderAcquisitionModule() {
      const activeDealId = state.tabs.acquisitionDeal;
      const body = byId('moduleBody');
      if (!activeDealId) {
      ...
      <div class="sheet-wrap" id="dealsWrap"></div>`;
        byId('dealsWrap').innerHTML = renderTable([{ key: 'asset', label: 'Asset' }, { key: 'buyer', label: 'Buyer', muted: true }, { key: 'seller', label: 'Seller', muted: true },
      ...
      const activeStage = state.tabs['stage_' + deal.id] ?? deal.stage;
    ...
    <div id="stageBody"></div>`;
      ...
      byId('pipeline').addEventListener('click', (e) => {
        const step = e.target.closest('[data-stage]'); if (!step) return;
        state.tabs['stage_' + deal.id] = Number(step.dataset.stage);
        renderModuleBody('acquisition');
      });
      renderStageBody(deal, activeStage);
    }
    function renderStageBody(deal, stageIdx) {
      ...
      function collect() {
        if (stageIdx === 0) { deal.meeting = { time: byId('fMeetTime').value, agent: byId('fMeetAgent').value, notes: byId('fMeetNotes').value } }
        if (stageIdx === 1) { deal.verification.notes = byId('fVerNotes').value; const f = byId('fVerFile').value; if (f) deal.verification.reports.push(f) }
        if (stageIdx === 2) { deal.loi.file = byId('fLoi').value }
        if (stageIdx === 3) { deal.escrow = { receipt: byId('fEscrowReceipt').value, status: byId('fEscrowStatus').value, amount: byId('fEscrowAmount').value, request: byId('fEscrowRequest').value } }
        if (stageIdx === 4) { deal.inspection = { reports: byId('fInspReports').value, summary: byId('fInspSummary').value } }
        if (stageIdx === 5) { deal.decision.status = byId('fDecision').value }
        if (stageIdx === 6) { deal.transfer.status = byId('fTransfer').value }
      }
    ...
    function renderFinanceModule() {
      ...
      <div class="sheet-wrap" id="txWrap"></div>`;
        ...
        wrap.innerHTML = renderTable([{ key: 'type', label: 'Type' }, { key: 'platform', label: 'Platform', muted: true }, { key: 'desc', label: 'Description', muted: true }, { key: 'amount', label: 'Amount' }, { key: 'date', label: 'Date', muted: true }], transactions, null);
        wrap.querySelectorAll('tbody tr').forEach((tr, i) => { tr.style.background = colorMap[transactions[i].color] || '' });
    ...
    <div class="modal-card"><h4>Revenue split</h4>
      ...
      <button class="btn btn-primary" id="saveTxBtn">Save transaction</button>`);
        byId('saveTxBtn').addEventListener('click', () => {
          const type = byId('txType').value.trim();
          if (!type) { showToast('Transaction type is required.'); return }
          transactions.unshift({ id: 't' + Date.now(), type, platform: byId('txPlatform').value, desc: byId('txDesc').value, to: byId('txTo').value, from: byId('txFrom').value, amount: byId('txAmount').value || '$0.00', date: 'Today', color: 'silver' });
    ...
    function renderProblemsModule() {
      ...
      byId('probTabs').addEventListener('click', (e) => { const b = e.target.closest('[data-tab]'); if (!b) return; state.tabs.problems = b.dataset.tab; renderModuleBody('problems') });
      ...
      if (tab === 'active') {
        ...
        byId('actProbWrap').innerHTML = renderTable([{ key: 'subject', label: 'Subject' }, { key: 'reporter', label: 'Reporter', muted: true }, { key: 'opened', label: 'Opened', muted: true }], complaintsActive, null);
      ...
      if (tab === 'solved') {
        ...
        byId('solvedWrap').innerHTML = renderTable([{ key: 'subject', label: 'Subject' }, { key: 'reporter', label: 'Reporter', muted: true }, { key: 'opened', label: 'Opened', muted: true }], complaintsSolved, null);
      ...
      if (tab === 'support') {
        ...
        byId('suppWrap').innerHTML = renderTable([{ key: 'name', label: 'Name' }, { key: 'contact', label: 'Contact', muted: true }, { key: 'date', label: 'Date', muted: true }, { key: 'help', label: 'How we helped', muted: true }], supportLog, null);
        ...
        <button class="btn btn-primary" id="saveSupportBtn">Save</button>`);
          byId('saveSupportBtn').addEventListener('click', () => {
            const name = byId('spName').value.trim();
            if (!name) { showToast('Name is required.'); return }
            supportLog.unshift({ id: 's' + Date.now(), name, contact: byId('spContact').value, date: 'Today', help: byId('spHelp').value });
    ...
    <button class="btn btn-primary" id="saveCompBtn" style="margin-top:12px">Save notes</button>`);
      byId('saveCompBtn').addEventListener('click', () => { c.notes = byId('compNotes').value; closeOverlay(); showToast('Notes saved.') });
    }

    /* ============================================================
       MODULE: ADMIN MANAGEMENT (full-screen, master only)
    ============================================================ */
    function openAdminManagementModule() {
      if (!authService.isMasterAdmin()) { adminRouter.renderForbidden(authService.getHomeRoute()); return; }
    ...
    function renderAdminManagementModule() {
      if (!authService.isMasterAdmin()) return;
      const body = byId('moduleBody');
      const roleCards = Object.entries(ROLE_DASHBOARDS).map(([roleKey, cfg]) => {
    ...
    <div class="sheet-wrap" id="adminMgmtWrap"></div>`;
      byId('adminMgmtWrap').innerHTML = renderTable([
      ...
      body.querySelectorAll('[data-view-role]').forEach((btn) => btn.addEventListener('click', () => {
        masterViewAdminDashboard({ role: btn.dataset.viewRole, name: btn.dataset.viewName });
      }));
    }
    ...
    function renderAuditLogsModule() {
      if (!authService.isMasterAdmin()) { adminRouter.renderForbidden(authService.getHomeRoute()); return; }
      const body = byId('moduleBody');
    ...
    <div class="sheet-wrap" id="auditLogsWrap"></div>`;
      byId('auditLogsWrap').innerHTML = renderTable([
    ...
    function openAddAdminModal() {
      ...
      const roleOptions = Object.entries(ROLE_LABELS).filter(([key]) => key !== 'MASTER_ADMIN').map(([key, label]) => `<option value="${key}">${esc(label)}</option>`).join('');
      ...
      function validateStep1(showErrors) {
        const name = byId('newAdminName')?.value.trim() || wizardState.form.name;
        const email = byId('newAdminEmail')?.value.trim() || wizardState.form.email;
        const role = byId('newAdminRole')?.value || wizardState.form.role;
        const status = byId('newAdminStatus')?.value || wizardState.form.status;
        wizardState.form = { name, email, role, status };
        let valid = true;
        const setErr = (id, msg, fieldValid) => {
      ...
      function getOtpValue(groupId) {
        ...
        if (!g) return '';
        return Array.from(g.querySelectorAll('.otp-input')).map((i) => i.value).join('');
      }
      function updateOtpButton(btnId, groupId) {
      ...
      function bindOtpInputs(groupId, btnId, onComplete) {
        const g = byId(groupId);
        if (!g) return;
        const inputs = Array.from(g.querySelectorAll('.otp-input'));
      ...
      function clearOtp(groupId) {
        const g = byId(groupId);
        if (!g) return;
        g.querySelectorAll('.otp-input').forEach((i) => { i.value = ''; i.classList.remove('filled'); });
        g.classList.remove('has-error');
        const err = byId(groupId + 'Err'); if (err) err.classList.remove('show');
      }
      function showOtpError(groupId, msg) {
        const g = byId(groupId);
        if (g) g.classList.add('has-error');
        const err = byId(groupId + 'Err');
        if (err) { err.textContent = msg; err.classList.add('show'); }
      }
      function startResendCountdown(btnId, seconds, resendFn) {
      ...
      function renderSuccessPanel(admin) {
      ...
      function bindStep1() {
        ...
        byId('newAdminRole')?.addEventListener('change', (e) => {
          byId('permPreview').innerHTML = renderPermPreview(e.target.value);
          updateStep1Button();
        });
      ...
      function bindStep2() {
        ...
        byId('masterOtpGroup')?.querySelector('.otp-input')?.focus();
      ...
      function bindStep3() {
        ...
        byId('newAdminOtpGroup')?.querySelector('.otp-input')?.focus();
  ...
  `);
    ...
    /* ============================================================
       GLOBAL BINDINGS
    ============================================================ */
    function showForbidden(title = '403 Forbidden', message = 'You do not have permission to access this module.') {
      openModal(`<h3>${esc(title)}</h3><div class="sub">${esc(message)}</div><button class="btn btn-ghost" data-close-modal>Close</button>`);
    }
    function bindGlobal() {
      byId('adminSidebar').addEventListener('click', (e) => {
      ...
      document.addEventListener('click', (e) => { if (e.target.closest('[data-close-modal]')) closeOverlay() });
      ...
      document.querySelectorAll('[data-open-module]').forEach((a) => { a.addEventListener('click', (e) => { e.preventDefault(); openModule(a.dataset.openModule) }); });
    }
